...
function instance$4($$self, $$props, $$invalidate) {
	...
	function Restore() {
		...
		if (gamestateJSON !== null) {
			...
			client.store.dispatch(reducer.sync(gamestate));
		}
	}
...
function instance$i($$self, $$props, $$invalidate) {
	...
	function OnLogClick(e) {
		...
		const renderedLogEntries = log.filter(e => e.action.type == reducer.MAKE_MOVE);
		...
		if ($$self.$$.dirty & /*$client, log, renderedLogEntries*/ 1540) {
			 {
				...
				$$invalidate(2, renderedLogEntries = log.filter(e => e.action.type == reducer.MAKE_MOVE));
